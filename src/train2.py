...
class ClipRsicdDataset(Dataset):
    def __len__(self) -> int:
        return len(self.captions_tokens)
    ...
    def __init__(
        ...
        dataset = load_dataset(
            "json", data_files="../data/rsicd_dataset.json", split="train"
        )
        max_seq_len = 0
        for i, row in enumerate(dataset):
            self.captions_tokens.append(torch.tensor(row["caption_tokens"]))
            self.prefixes.append(torch.tensor(row["clip_embedding"]))
            max_seq_len = max(max_seq_len, self.captions_tokens[-1].shape[0])
...
def main():
    ...
    if args.only_prefix:
        ...
        sys.stdout.flush()
    train(
        dataset, model, args, DEVICE, output_dir=args.out_dir, output_prefix=args.prefix
    )
